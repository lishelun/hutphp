...
class TxcosStorage extends Storage
{
    ...
    {
        // 读取配置文件
        $this->point     = config('storage.txcos.point');
        $this->bucket    = config('storage.txcos.bucket');
        $this->secretId  = config('storage.txcos.access_key');
        $this->secretKey = config('storage.txcos.secret_key');
        // 计算链接前缀
        $type   = strtolower(config('storage.txcos.protocol'));
        $domain = strtolower(config('storage.txcos.domain'));
        if ( $type === 'auto' ) {
            $this->prefix   = "//{$domain}";
            $this->protocol = 'https';
     ...
     * @return array
     */
    private function headerSign(string $method , string $source): array
    {
        $header = [];
        // 1.生成 KeyTime
        $startTimestamp = time();
        ...
        [$parse_url , $urlParamList , $httpParameters] = [parse_url($source) , '' , ''];
